...
## 10.11 决策记录 6（2026-08-04，工程收尾拍板并入文）
| # | 决策 | 拍板 | 落点 |
|---|---|---|---|
| ㉔ | 简历案例说明 | **压缩版入 docs**（300 字），初稿不落盘 | docs/简历案例说明_压缩版.md |
| ㉕ | 真实项目资料时机 | **录屏用示例资料，真实资料面试后补**（示例已验证稳定，录屏求稳） | project_kb/data/ 待补 |
| ㉖ | 工程收尾 | **README 加 Gitee 链接 + CHANGELOG + 打 v0.1.0 tag** | README / CHANGELOG / git tag |
| ㉗ | 条文级溯源等增强项 | **缓，列为 v2 未来规划**（MVP 已够面试展示，锁边界防蔓延） | §11 未来规划 |

## 11. 未来规划（v2+，本 MVP 明确不做，锁定边界）

> 以下增强项均为 v2 及以后考虑，MVP 不实现。防止范围蔓延——开发中如有人提出这些功能，直接拒绝。

| 增强项 | 说明 | 预计版本 |
|--------|------|----------|
| 条文级溯源 | 规范路从「文件级」细化到「条文级」（需母体规范库按条文切片，重构规范数据源） | v2 |
| 全专业规范库扩展 | 当前规范路复用 KG_Test 母体（HVACR 本体 478 节点/417 关系），扩展暖通以外专业 | v2+ |
| 多项目知识资产沉淀 | 项目背景资料跨项目复用、知识沉淀为中台 | v2+ |
| 企业级知识中台 | 多用户/权限/审计，演进为内部 SaaS | v3 |
| 真实项目资料入库 | 用户真实脱敏资料补进 `project_kb/data/`，跑完整验证 | 面试后随时 |
